...
(function ($, Drupal, { tabbable, isTabbable }) {
  ...
  $.extend(TabbingManager.prototype, {
    constrain(elements, { trapFocus = false } = {}) {
      ...
      for (let i = 0; i < il; i++) this.stack[i].deactivate();
    ...
    release() {
      ...
      while (toActivate >= 0 && this.stack[toActivate].released) toActivate--;
      this.stack.splice(toActivate + 1);
      if (toActivate >= 0) this.stack[toActivate].activate();
    },
    activate(tabbingContext) {
      ...
      if ($hasFocus.length === 0) $hasFocus = $set.eq(0);
    ...
    deactivate(tabbingContext) {
      ...
      for (let i = 0; i < il; i++) this.restoreTabindex($set.eq(i), level);
    },
    ...
    restoreTabindex($el, level) {
      ...
      if (tabInfo && tabInfo[level]) {
        ...
        if (data.tabindex) $el[0].setAttribute('tabindex', data.tabindex);
        else $el[0].removeAttribute('tabindex');
        if (data.autofocus) $el[0].setAttribute('autofocus', 'autofocus');
        if (level === 0) $el.removeData('drupalOriginalTabIndices');
        else {
  ...
  if (Drupal.tabbingManager) return;
...
var app = {
  ...
  isCordova: false,
  includeNavbar: false,
  includeMaterialMenu: false,
  includePush: false,
  pushAppId: null,
  androidAppId: null,
  includeFileHandler: false,
  ...
  init: function (_options) {
    app.trace('initializing');
    if (_options) {
      if (_options.debug && _options.debug == true) app.debug = true;
      if (_options.reverseDns) app.reverseDns = _options.reverseDns;
      if (_options.useLoader && _options.useLoader == true)
        app.useLoader = true;
      if (_options.listenForResize && _options.listenForResize == true)
        app.listenForResize = true;
      if (_options.pageTransition) app.pageTransition = _options.pageTransition;
      if (_options.pageDirection) app.pageDirection = _options.pageDirection;
      if (_options.swipeEnabled && _options.swipeEnabled == true) {
        app.swipeEnabled = true;
        if (app.pageDirection == 'lr')
          appSwipe.init({
            left: function () {
              app.nextPage();
            },
      ...
      if (_options.name) app.name = _options.name;
      if (_options.pages) app.pages = _options.pages;
      if (_options.includeCharts) app.includeCharts = _options.includeCharts;
      if (_options.includeNavbar && _options.includeNavbar == true) {
        app.includeNavbar = true;
        navbar.init({
          name: app.name,
          type: 'static',
          linksRight: true,
          directLink: false,
          linkFunction: function (_page) {
            app.loadPage(_page);
          },
      ...
      if (
        _options.includeMaterialMenu &&
        _options.includeMaterialMenu == true
      ) {
        app.includeMaterialMenu = true;
        materialMenu.init({
          name: app.name,
          directLink: false,
          linkFunction: function (_page) {
          ...
          transition: 'slide',
          linkTransition: 'zoom',
          alwaysIncludeBtn: true,
          pages: app.pages,
        });
      }
      if (_options.includePush) {
        app.includePush = true;
        if (_options.pushAppId) app.pushAppId = _options.pushAppId;
        if (_options.androidAppId) app.androidAppId = _options.androidAppId;
      }
      if (_options.includeFileHandler)
        if (app.reverseDns) {
          app.includeFileHandler = true;
          fileHandler.init(app.reverseDns);
        }
      if (_options.isApp && _options.isApp == true) {
        app.isApp = true;
        if (_options.isCordova && _options.isCordova == true) {
          app.isCordova = true;
          app.bindAppEvents();
        } else app.ready();
      } else {
    ...
    } else console.log('No initialization settings provided');
  },
  bindAppEvents: function () {
    app.trace('binding cordova events');
    document.addEventListener('deviceready', app.ready, false);
  },
  ready: function () {
    app.trace('ready');
    if (app.includePush && app.isCordova)
      push.init({
        appId: app.pushAppId,
        androidProjectId: app.androidAppId,
        action: function () {
          console.log('push notification received while app is running');
        },
      });
    if (app.includeMaterialMenu && app.isCordova)
      document.addEventListener(
        'menubutton',
        function () {
          materialMenu.toggleMenu();
        },
        false
      );
    if (app.listenForResize) {
      app.onResize();
  ...
  loadPage: function (_pagenum, _direction) {
    _page = app.pages[_pagenum];
    if (_page && _pagenum != app.currentPage) {
      app.trace('load page: ' + _page.name);
      if (app.useLoader) app.showLoader('Loading ' + _page.name);
      $.ajax({
        url: 'pages/' + _page.file,
        cache: false,
        success: function (result) {
          app.trace('page loaded');
          if (!_direction) {
            _direction = app.getPageDirection(_pagenum);
            if (app.pageTransition == 'flip')
          ...
          var _inAnim = 'animated ' + app.pageTransition + 'In' + _direction;
          var _outAnim = 'animated ' + app.pageTransition + 'Out';
          if (_direction == 'Left')
            if (app.pageTransition != 'flip') _outAnim += 'Right';
            else _outAnim += 'Y';
          else if (_direction == 'Right')
            if (app.pageTransition != 'flip') _outAnim += 'Left';
            else _outAnim += 'Y';
          else if (_direction == 'Up')
            if (app.pageTransition != 'flip') _outAnim += 'Up';
            else _outAnim += 'X';
          else {
            if (_direction == 'Down')
              if (app.pageTransition != 'flip') _outAnim += 'Down';
              else _outAnim += 'X';
          }
          var _w = 1024;
          $('.main').html(result);
          $('.main .page.page-' + app.currentPage)
            .parent()
            .removeClass(_inAnim);
          $('.main .page.page-' + app.currentPage)
            .parent()
          ...
          setTimeout(
            function () {
              $('.main .page.page-' + app.currentPage)
                .parent()
                .removeClass('animated ' + app.pageTransition + 'OutLeft');
              $('.main .page.page-' + app.currentPage)
                .parent()
                .removeClass('animated ' + app.pageTransition + 'OutRight');
              $('.main .page.page-' + app.currentPage)
                .parent()
                .removeClass('animated ' + app.pageTransition + 'InLeft');
              $('.main .page.page-' + app.currentPage)
                .parent()
                .removeClass('animated ' + app.pageTransition + 'InRight');
              $('.main .page.page-' + app.currentPage)
                .parent()
                .removeClass('animated ' + app.pageTransition + 'OutUp');
              $('.main .page.page-' + app.currentPage)
                .parent()
                .removeClass('animated ' + app.pageTransition + 'OutDown');
              $('.main .page.page-' + app.currentPage)
                .parent()
                .removeClass('animated ' + app.pageTransition + 'InUp');
              $('.main .page.page-' + app.currentPage)
                .parent()
                .removeClass('animated ' + app.pageTransition + 'InDown');
              $('.main .page.page-' + app.currentPage)
                .parent()
                .removeClass('animated ' + app.pageTransition + 'OutY');
              $('.main .page.page-' + app.currentPage)
                .parent()
                .removeClass('animated ' + app.pageTransition + 'OutX');
              $('.main .page.page-' + app.currentPage)
                .parent()
                .removeClass('animated ' + app.pageTransition + 'InY');
              $('.main .page.page-' + app.currentPage)
                .parent()
                .removeClass('animated ' + app.pageTransition + 'InX');
              $('.main .page.page-' + app.currentPage)
                .parent()
                .removeClass('animated');
              $('.main .page.page-' + app.currentPage).remove();
              $('.main .page.page-' + _pagenum).unwrap();
              $('.new-page').each(function () {});
              app.currentPage = Number(_pagenum);
              if (app.useLoader) app.removeLoader();
              $('body').scrollTop(0);
              window[_page.id].init();
              app.ui.init();
              if (app.includeCharts == true || app.includeCharts == 'true')
                autoChart();
            },
            app.pageTransition != 'none' ? 1000 : 250
          );
        },
      });
    }
  ...
  nextPage: function () {
    ...
    if (app.currentPage < app.pages.length - 1)
      app.loadPage(app.currentPage + 1);
  },
  prevPage: function () {
    app.trace('previous page');
    if (app.currentPage > 0) app.loadPage(app.currentPage - 1);
  },
  getPageDirection: function (_nextPage) {
    var _ret;
    if (app.pageDirection == 'lr')
      if (_nextPage > app.currentPage) _ret = 'Right';
      else _ret = 'Left';
    else {
      if (app.pageDirection == 'ud')
        if (_nextPage > app.currentPage) _ret = 'Up';
        else _ret = 'Down';
    }
    return _ret;
  },
  scrollTo: function (_anchor) {
    var $anchor = $(String(_anchor));
    $('html, body')
      .stop()
      .animate(
  ...
  ui: {
    initActivated: function () {
      $('.activated li').unbind();
      $('.activated li').bind('click', function () {
        $('.activated li').removeClass('active');
        $(this).addClass('active');
      });
    },
    initExpandable: function () {
      $('.expandable li .expander').unbind();
      $('.expandable li .expander').bind('click', function () {
    ...
    initScrollTo: function () {
      ...
      $('.page-scroll').bind('click', function (event) {
        app.scrollTo($(this).attr('rel-page-scroll'));
        event.preventDefault();
      });
    },
    initPageLinks: function () {
      $('*[app-page]').unbind('click');
      $('*[app-page]').bind('click', function (event) {
        var _page = $(this).attr('app-page');
        if ($.isNumeric(_page)) app.loadPage(_page);
        else if (_page.toLowerCase() == 'nextpage') app.nextPage();
        else if (_page.toLowerCase() == 'prevpage') app.prevPage();
        else {
          for (var i = 0; i < app.pages.length; i++)
            if (app.pages[i].name == _page || app.pages[i].id == _page)
              app.loadPage(i);
        }
        event.preventDefault();
      });
    ...
    init: function () {
      app.trace('initializing UI');
      app.ui.initActivated();
      app.ui.initExpandable();
      app.ui.initDropdowns();
      app.ui.initScrollTo();
      app.ui.initPageLinks();
    },
  },
  showLoader: function (_msg) {
    ...
    var _loader =
      '<div class="loader" style="position:fixed;z-index:5001;top:0px;left:0px;bottom:0px;right:0px;width:100%;height:100%;background:rgba(0,0,0,0.65) !important;padding:25px;padding-top:100px;font-size:12pt;color:#ffffff;font-weight:bold;text-align:center;text-shadow:none;"><img src="/themes/custom/doseview/images/loader.gif" width="32" height="32"/><br/><p class="loader-details">' +
      _msg +
      '</p></div>';
    $('.loader').remove();
    $('body').append(_loader);
  },
  updateLoader: function (_str) {
    $('.loader-details').html(_str);
  },
  removeLoader: function () {
    $('.loader').remove();
  },
  alert: function (_string) {
    var _overlay =
      '<div class="panel-overlay panel-overlay-alert animated fadeIn" onclick="javascript:app.removeAlert();">';
    _overlay += '	<div class="panel panel-danger">';
    _overlay += '		<div class="panel-heading">';
    _overlay += '			<h3 class="panel-title">Oops!</h3>';
    _overlay += '		</div>';
    _overlay += '		<div class="panel-body">';
    _overlay += _string;
    _overlay += '		</div>';
    _overlay += '	</div>';
    _overlay += '</div>';
    ...
    window.alertEvt;
    clearTimeout(window.alertEvt);
    window.alertEvt = setTimeout(function () {
      app.removeAlert();
    }, 3000);
  },
  removeAlert: function () {
    $('.panel-overlay-alert .panel').removeClass('animated');
    $('.panel-overlay-alert .panel').removeClass('fadeInDown');
    $('.panel-overlay-alert .panel').addClass('fadeOutUp');
    $('.panel-overlay-alert').removeClass('animated');
    $('.panel-overlay-alert').removeClass('fadeIn');
    $('.panel-overlay-alert').addClass('animated fadeOut');
    window.alertRemEvt;
    clearTimeout(window.alertRemEvt);
    window.alertRemEvt = setTimeout(function () {
      $('.panel-overlay-alert').remove();
    }, 500);
  },
  checkNull: function (_str) {
    if (_str && String(_str != 'null') && String(_str != 'undefined'))
      return false;
    else return true;
  },
  onResize: function () {
    app.trace('app resized');
  },
  trace: function (_str) {
    if (app.debug == true) console.log(_str);
  },
};
var analyticsTracker = {
  code: 'UA-50488016-2',
  bindMenu: function () {
    $('.nav-btns li').bind('click', function () {
      switch ($(this).attr('rel-page')) {
        case '0':
          analyticsTracker.trigger('Top Nav Overview Button');
          break;
        case '1':
          analyticsTracker.trigger('Top Nav Benchmark Button');
          break;
        case '2':
          analyticsTracker.trigger('Top Nav Calculator Button');
      }
    });
  },
  bindOverview: function () {
    $('.slide-3 .blue-button').bind('click', function () {
      analyticsTracker.trigger(
        'Overview Slide View National Benchmark Data Button'
      ),
        setTimeout(function () {
          window.scrollTo(0, 0);
        }, 50);
    }),
      $('.slide-4 .blue-button').bind('click', function () {
        analyticsTracker.trigger('Overview Slide Start The Calculator Button'),
          setTimeout(function () {
            window.scrollTo(0, 0);
          }, 50);
      });
  },
  bindBenchmark: function () {
    $('.grey-divide .blue-button').bind('click', function () {
      analyticsTracker.trigger('Benchmark Start The Calculator Button'),
        setTimeout(function () {
          window.scrollTo(0, 0);
        }, 50);
    });
  ...
  bindCalc: function () {
    $('.btn-calculate').bind('click', function () {
      analyticsTracker.trigger('Calculator View Results Button'),
        setTimeout(function () {
          window.scrollTo(0, 0);
        }, 50);
    });
...
jQuery(function ($) {
  app.init({
    name: 'ALAI Dose Mix Calculator',
    debug: !1,
    isApp: !0,
    isCordova: !1,
    includeNavbar: !1,
    includeMaterialMenu: !1,
    includePush: !1,
    pushAppId: '',
    androidAppId: '',
    includeFileHandler: !1,
    swipeEnabled: !1,
    reverseDns: '',
    pageTransition: 'none',
    pageDirection: 'ud',
    ...
    $('*[rel-page]').click(function () {
      app.loadPage($(this).attr('rel-page')),
        $('[rel-page]').removeClass('active'),
        $(this).addClass('active');
    }),
  ...
  (m.flatten = function (n, t) {
    ...
    (m.indexOf = function (n, t, r) {
      ...
      if ('number' == typeof r) e = 0 > r ? Math.max(0, u + r) : r;
        ...
        if (r && u) return (e = m.sortedIndex(n, t)), n[e] === t ? e : -1;
      }
